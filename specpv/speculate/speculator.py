...
class Speculator(nn.Module):
    def __init__(
        ...
        self.ea_layer = self._init_draft_layer(
            config, total_token, depth, top_k, threshold
        )
    ...
    def from_pretrained(
        ...
        **kwargs,
        ...
        if total_token == -1:
            total_token = profile_total_tokens(model)
    ...
    def forward(
        ...
        input_ids=None,
        attention_mask=None,
        full_past_key_values=None,
        partial_past_key_values=None,
        output_orig=False,
        position_ids=None,
    ):
...
def profile_total_tokens(model: Speculator):
    ...
    for i, length in enumerate(cans):
    ...
    best_len = cans[times.index(min(times))]
    return best_len
